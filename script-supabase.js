...
async function loadProjects(selectId=null, listId=null){
  ...
  if (listId){
    const ul = el(listId);
    if (ul){
      ul.innerHTML =  (data || []).map(p => `<li>${p.name}
        
        <!-- Edit button -->

        <button class="icon-btn edit" data-tooltip="Edit Nama" onclick="editProjectName('${p.id}', '${p.name}')">
        ...
        <button class="icon-btn delete" data-tooltip="Hapus Drawing" onclick="hapusProject('${p.id}')">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
async function editProjectName(id, oldName){
  const newName = prompt("Ubah nama Project:", oldName);
  if (!newName || newName.trim() === "" || newName === oldName) return;

  try {
    let { error } = await sb.from("projects").update({ name: newName }).eq("id", id);
    if (error) throw error;
    alert("Nama Project berhasil diubah.");
    await loadProjects(); // refresh daftar
  } catch(err){
    console.error(err);
...
async function tampilkanData(kategori, projectId){
  ...
  const rows = data.map(d => {
    ...
    return `
      <tr>
        <td class="tdcenter">${d.joint ?? "-"}</td>
        <td class="tdcenter">${drawingCell}</td>
        <td class="tdcenter">${d.material ?? "-"}</td>
        <td class="tdcenter">${d.ukuran ?? "-"}</td>
        <td class="tdcenter">${d.welder ?? "-"}</td>
        <td class="tdcenter">${d.tanggal ?? "-"}</td>
        <td class="tdcenter">${d.shift ?? "-"}</td>
        <td class="tdcenter">${statusCell}</td>
        ${extraCell}
        <td Class="alltd">${d.catatan ?? "-"}</td>
        ${auditCell}
        <td class="tdcenter">
          ${
